...
class _MessageState extends State<Message> {
  ...
  void initState() {
   // checkseenmessages();
    ...
    super.initState();
     ...
     _scrollcon.addListener(changeSelector);
  ...
  changeSelector() {
    _scrollcon.animateTo( _scrollcon.position.maxScrollExtent,  curve: Curves.easeIn, duration: Duration(milliseconds: 10));
  ...
  test(messagePath) {
   ...
   var recipt= Firestore.instance.collection('$messagePath/chats').where('sender', isEqualTo: int.parse(widget.id)).snapshots();
      print('THis is reciver'+ widget.id);
    recipt.listen((event) { 
      for(int i=0; i< event.documents.length; i++){
        print(event.documents[i].documentID);
        var docid= event.documents[i].documentID;
         Firestore.instance.collection('$messagePath/chats').document('$docid').updateData({ 
            'delivered': true,
            'seen': true
         });
      }
    });
            //commented seen for performance check 6/7/20
    ...
    data.listen((event) {
      ...
      setState(() {
          path = Firestore.instance
            .collection(
                '/contacts/${widget.own_mobile}/message/${widget.id}/chats')
        ...
        //     .snapshots(); 
              ...
              message_path= '/contacts/${widget.own_mobile}/message/${widget.id}';
...
//  //websocket test ends date 26.5.20
                Row(
                  ...
                  print(message_path);
                          ...
                          //  var name= Firestore.instance.collection('contacts').document('${widget.own_mobile}').get();
                          // name.then((value) async {
                          //  var _name= value.data['own_name'];
